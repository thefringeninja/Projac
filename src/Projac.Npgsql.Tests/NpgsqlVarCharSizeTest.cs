...
{
    [TestFixture]
    public class NpgsqlVarCharSizeTest
    {
        [Test]
        public void MaxReturnsExpectedResult()
        {
            Assert.That(NpgsqlVarCharSize.Max, Is.EqualTo(new NpgsqlVarCharSize(-1)));
        }
        ...
        {
            var sut = SutFactory();
            Assert.That(sut, Is.InstanceOf<IEquatable<NpgsqlVarCharSize>>());
        }
        ...
        {
            NpgsqlVarCharSize sut = 123;

            Assert.That(sut, Is.EqualTo(SutFactory(123)));
        }

        private static NpgsqlVarCharSize SutFactory()
        {
            return SutFactory(new Random().Next(-1, 8000));
        }

        private static NpgsqlVarCharSize SutFactory(int value)
        {
