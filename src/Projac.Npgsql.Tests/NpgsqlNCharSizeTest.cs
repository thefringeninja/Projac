...
{
    [TestFixture]
    public class NpgsqlNCharSizeTest
    {
        [Test]
        public void MaxReturnsExpectedResult()
        {
            Assert.That(NpgsqlNCharSize.Max, Is.EqualTo(new NpgsqlNCharSize(-1)));
        }
        ...
        {
            var sut = SutFactory();
            Assert.That(sut, Is.InstanceOf<IEquatable<NpgsqlNCharSize>>());
        }
        ...
        {
            NpgsqlNCharSize sut = 123;

            Assert.That(sut, Is.EqualTo(SutFactory(123)));
        }

        private static NpgsqlNCharSize SutFactory()
        {
            return SutFactory(new Random().Next(-1, 4000));
        }

        private static NpgsqlNCharSize SutFactory(int value)
        {
