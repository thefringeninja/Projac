...
{
    [TestFixture]
    public class NpgsqlNVarCharSizeTest
    {
        [Test]
        public void MaxReturnsExpectedResult()
        {
            Assert.That(NpgsqlNVarCharSize.Max, Is.EqualTo(new NpgsqlNVarCharSize(-1)));
        }
        ...
        {
            var sut = SutFactory();
            Assert.That(sut, Is.InstanceOf<IEquatable<NpgsqlNVarCharSize>>());
        }
        ...
        {
            NpgsqlNVarCharSize sut = 123;

            Assert.That(sut, Is.EqualTo(SutFactory(123)));
        }

        private static NpgsqlNVarCharSize SutFactory()
        {
            return SutFactory(new Random().Next(-1, 4000));
        }

        private static NpgsqlNVarCharSize SutFactory(int value)
        {
