...
{
    [TestFixture]
    public class NpgsqlCharSizeTest
    {
        [Test]
        public void MaxReturnsExpectedResult()
        {
            Assert.That(NpgsqlCharSize.Max, Is.EqualTo(new NpgsqlCharSize(-1)));
        }
        ...
        {
            var sut = SutFactory();
            Assert.That(sut, Is.InstanceOf<IEquatable<NpgsqlCharSize>>());
        }
        ...
        {
            NpgsqlCharSize sut = 123;

            Assert.That(sut, Is.EqualTo(SutFactory(123)));
        }

        private static NpgsqlCharSize SutFactory()
        {
            return SutFactory(new Random().Next(-1, 8000));
        }

        private static NpgsqlCharSize SutFactory(int value)
        {
