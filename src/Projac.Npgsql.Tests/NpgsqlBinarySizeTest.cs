...
{
    [TestFixture]
    public class NpgsqlBinarySizeTest
    {
        ...
        {
            var sut = SutFactory();
            Assert.That(sut, Is.InstanceOf<IEquatable<NpgsqlBinarySize>>());
        }
        ...
        {
            NpgsqlBinarySize sut = 123;

            Assert.That(sut, Is.EqualTo(SutFactory(123)));
        }

        private static NpgsqlBinarySize SutFactory()
        {
            return SutFactory(new Random().Next(-1, 8000));
        }

        private static NpgsqlBinarySize SutFactory(int value)
        {
