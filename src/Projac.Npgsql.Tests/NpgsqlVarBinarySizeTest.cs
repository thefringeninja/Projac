...
{
    [TestFixture]
    public class NpgsqlVarBinarySizeTest
    {
        [Test]
        public void MaxReturnsExpectedResult()
        {
            Assert.That(NpgsqlVarBinarySize.Max, Is.EqualTo(new NpgsqlVarBinarySize(-1)));
        }
        ...
        {
            var sut = SutFactory();
            Assert.That(sut, Is.InstanceOf<IEquatable<NpgsqlVarBinarySize>>());
        }
        ...
        {
            NpgsqlVarBinarySize sut = 123;

            Assert.That(sut, Is.EqualTo(SutFactory(123)));
        }

        private static NpgsqlVarBinarySize SutFactory()
        {
            return SutFactory(new Random().Next(-1, 8000));
        }

        private static NpgsqlVarBinarySize SutFactory(int value)
        {
